...
@push('styles')
<style>
/* Modern Grid UI (Theme Compatible) */
.header-actions {
    display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem;
    background: var(--surface); border: 1px solid var(--border); border-radius: var(--r-xl); padding: 1.5rem; box-shadow: var(--shadow-sm);
}
...
/* Kebab Menu Dropdown */
.kebab-menu { position: relative; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; color: rgba(255,255,255,0.7); transition: color var(--t); }
.kebab-menu:hover { color: #fff; }
.grid-card:hover .kebab-menu-bg { opacity: 1; }
.kebab-menu-bg { opacity: 0.5; transition: 0.2s; position:absolute; top:12px; right:12px; background:rgba(0,0,0,0.6); backdrop-filter:blur(4px); border-radius:8px; padding:4px; z-index:10; }
...
/* Grid Layout */
.grid-wrap {
    display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 1.5rem; margin-top: 1rem;
}
.grid-card {
    background: var(--surface); border: 1px solid var(--border); border-radius: var(--r-xl); overflow: hidden; position: relative;
    box-shadow: var(--shadow-sm); transition: all 0.2s; animation: cmsFadeIn 0.3s cubic-bezier(0.16, 1, 0.3, 1) both;
}
.grid-card:hover { transform: translateY(-4px); box-shadow: var(--shadow); border-color: rgba(var(--accent-rgb), 0.3); }
...
<div class="header-actions">
    <div>
        <span class="badge badge-blue">Gallery: {{ $table }}</span>
...
@if($rows->isEmpty())
    <div style="text-align:center;padding:3rem;color:var(--text-3);background:var(--surface);border:1px solid var(--border);border-radius:var(--r-xl);">
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
             style="margin:0 auto 1rem;display:block;opacity:0.3;">
            <rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/>
        </svg>
        <p style="font-size:0.9rem;">No media or records found.</p>
        ...
        @foreach($rows as $row)
            @php 
                ...
                foreach($columns as $col) {
                    if(in_array($col, ['id', 'document_url', 'created_at', 'updated_at'])) continue;
                    $val = $arr[$col];
                    
                    if(!$imageField && (str_contains(strtolower($col), 'image') || str_contains(strtolower($col), 'photo') || str_contains(strtolower($col), 'thumbnail') || str_contains(strtolower($col), 'cover') || preg_match('/\.(jpg|jpeg|png|gif|webp)$/i', (string)$val))) {
                        $imageField = $col;
                    } elseif(!$titleField && in_array(gettype($val), ['string', 'integer']) && strlen((string)$val) < 100 && !is_numeric($val) || $col === 'title' || $col === 'name') {
                        $titleField = $col;
                    } else {
                        $secondaryFields[$col] = $val;
                    }
                }
                ...
                if(!$titleField) { 
                    foreach($columns as $col) {
                        if($col !== $imageField && !in_array($col, ['id', 'document_url', 'created_at', 'updated_at'])) {
                            $titleField = $col; break;
                        }
                    }
                }
            @endphp
            ...
            <div class="grid-card">
                ...
                @if($imageField && $arr[$imageField])
                    @php
                        $rawImg = $arr[$imageField];
                        if (is_string($rawImg) && str_starts_with(trim($rawImg), '[') && str_ends_with(trim($rawImg), ']')) {
                    ...
                    @endphp
                    <div style="height:200px; width:100%; background:var(--surface-2); overflow:hidden; border-bottom:1px solid var(--border);">
                        @if(filter_var($rawImg, FILTER_VALIDATE_URL) || str_starts_with($rawImg, '/'))
                            <img src="{{ $rawImg }}" style="width:100%; height:100%; object-fit:cover;" onerror="this.outerHTML='<div style=\'width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:3rem;\'>📄</div>'">
                        @else
                            <img src="{{ url($rawImg) }}" style="width:100%; height:100%; object-fit:cover;" onerror="this.outerHTML='<div style=\'width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:3rem;\'>📄</div>'">
                        @endif
                    </div>
                @else
                    <div style="height:200px; width:100%; background:linear-gradient(135deg, var(--surface-2), var(--surface-3)); display:flex; align-items:center; justify-content:center; border-bottom:1px solid var(--border);">
                         <span style="font-size:3rem; opacity:0.1; filter:grayscale(1);">
                            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
                         </span>
                    </div>
                @endif
                
                <div style="padding:1.25rem;">
                    <h3 style="margin:0 0 0.5rem; font-size:1.1rem; color:var(--text); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="{{ $titleField ? $arr[$titleField] : '' }}">
                        {{ $titleField ? ($arr[$titleField] ?: 'Untitled') : 'Record #'.$arr['id'] }}
                    </h3>
                    
                    <div style="display:flex; flex-direction:column; gap:6px; font-size:0.85rem; color:var(--text-3);">
                        @php $count = 0; @endphp
                        @foreach($secondaryFields as $k => $v)
                            @if($count < 2)
                                <div style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="{{ (string)$v }}">
                                    <strong>{{ ucwords(str_replace('_', ' ', $k)) }}:</strong> 
                                    @php $valStr = (string)$v; @endphp
                                    {{ strlen($valStr) > 40 ? substr($valStr, 0, 40).'...' : ($valStr ?: '—') }}
                                </div>
                            @endif
                            @php $count++; @endphp
                        @endforeach
